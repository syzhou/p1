...
int remove_Meeting_participant(struct Meeting* meeting_ptr, const struct Person* person_ptr) {
	void *itemPtr = OC_find_item(meeting_ptr->participants, (void*)person_ptr);
	if (itemPtr) {
		OC_delete_item(meeting_ptr->participants, itemPtr);
		return 0;
	} else {
		return 1;
	}
...
void save_Meeting(const struct Meeting* meeting_ptr, FILE* outfile) {
	...
	OC_apply_arg(meeting_ptr->participants, (OC_apply_fp_t)savePersonLastname, outfile);
}
