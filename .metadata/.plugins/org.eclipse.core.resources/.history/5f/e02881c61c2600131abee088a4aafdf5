...
void* OC_find_item_arg(const struct Ordered_container* c_ptr, const void* arg_ptr, OC_find_item_arg_fp_t fafp) {
	...
	while (itemPtrItr = itemPtrItr->next) {
		if (!c_ptr->comp_func(arg_ptr, OC_get_data_ptr(itemPtrItr))) {
			return itemPtrItr;
		} else if (c_ptr->comp_func(arg_ptr, OC_get_data_ptr(itemPtrItr))< 0) {
			return NULL;
		}
	}
	return NULL;
...
struct LL_Node* OCFindItemLesserThan(const struct Ordered_container* cPtr, const void* dataPtr) {
	...
	struct LL_Node* itemPtrItr = cPtr->first;
	while(itemPtrItr) {
